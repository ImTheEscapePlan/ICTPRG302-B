...
parser.add_argument(
    "job", type=str, help="name of backup job to use (job1, job2, job3)"
)

args = parser.parse_args()

smtp = {
    "sender": "cooperlehman3108@gmail.com",
...
if __name__ == "__main__":
    ...
    def job1():
        ...
        if not source.exists():
            print(
                f"Error: Source directory '{source}' does not exist. Check backupcfg.py"
            )
            logging.error(
        ...
        for file_path in source.rglob("*"):
            if file_path.is_file():
                try:
                    ...
                    # append the timestamp to every directory in the relative path
                    # e.g., 'folder1/folder2' becomes 'folder1_2026.../folder2_2026...'
                    new_parts = [
                        f"{part}_{timestamp}" for part in relative_path.parent.parts
                    ]
                    new_relative_parent = Path(*new_parts)

                    # determine the final destination path
                    target_subdir = dest / new_relative_parent
                    ...
                    # Log the full new relative path so you can see the directory changes
                    print(
                        f"Successfully backed up: {relative_path} -> {new_relative_parent / new_filename}"
                    )
                    logging.info(
    ...
    if args.job == "job1":
        ...
        if sched == "y":
            print(
                f"scheduled time is {backupcfg.SchedTime}, if this is not correct, please edit backupcfg before running"
            )
        ...
        if sched == "y":
            print(
                f"scheduled time is {backupcfg.SchedTime}, if this is not correct, please edit backupcfg"
            )
        ...
        logging.error(
            f"FAIL: {args.job} is not a supported job, please try job1, job2 or job3"
        )
